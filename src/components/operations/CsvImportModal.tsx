...
export function CsvImportModal({ open, onOpenChange, onSuccess }: CsvImportModalProps) {
        ...
        Papa.parse<CsvRow>(file, {
            ...
            complete: (results) => {
                if (results.data && results.data.length > 0) {
                    track('import.csv_started', { row_count: results.data.length }, 'CsvImportModal');
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const { error } = await supabase.from('assets').insert(payload as never);  // Type assertion due to strict enum types
            if (error) throw error;
