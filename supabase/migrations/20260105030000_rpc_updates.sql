...
-- 1. ISSUE RESERVATION
-- Drop potential legacy signature to prevent ambiguity (Ambiguous function error)
...
BEGIN
    ...
    IF v_reservation.status != 'confirmed' THEN
         ...
         USING ERRCODE = 'P0002'; -- Custom code
    END IF;
    ...
    IF NOT (
        ...
        USING ERRCODE = 'P0003'; -- Custom code PAYMENT_REQUIRED
    END IF;
    ...
    IF v_needed_count > 0 THEN
        -- Find available assets for this variant
        ...
        FOR v_asset_id IN 
            ...
            FOR UPDATE SKIP LOCKED -- Skip booked assets locked by concurrent tx
        LOOP
    ...
    IF v_assigned_count < v_quantity THEN
        RAISE EXCEPTION 'Not enough assets available to issue reservation (Needed %, Found %)', v_quantity, v_assigned_count
        USING ERRCODE = 'P0005'; -- NO_ASSETS
    END IF;
    ...
    SELECT count(*) INTO v_updated_count FROM updated_assets;
...
BEGIN
    ...
    LOOP
        ...
        SET status = v_new_status,
            location = 'Warehouse', -- Return to stock
            updated_at = now()
