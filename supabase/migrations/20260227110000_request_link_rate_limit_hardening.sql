...
-- =============================================================================
COMMENT ON TABLE public.request_submit_rate_buckets IS
  'Fixed-window rate limit buckets (window anchored to first request in period). Cleanup: window_end < now() - 1 hour. bucket_key = type:identifier, 64-char hex (ip:<ip_hash>, token:<token_hash>).';
...
DECLARE
  ...
  v_retry_sec int;
BEGIN
  -- bucket_key: type:identifier (unique, stable). Full 64-char hex for ip and token. No plaintext IP stored.
...
REVOKE ALL ON FUNCTION public.submit_request_public(text, text, text, text, text, date, date, uuid, text, text) FROM PUBLIC;
...
ALTER TABLE public.reservation_requests
  ADD CONSTRAINT reservation_requests_status_converted_check CHECK (
    ...
    OR
    (status = 'rejected' AND rejected_at IS NOT NULL AND converted_reservation_id IS NULL)
    OR
    (status = 'pending' AND converted_reservation_id IS NULL)
  );

COMMENT ON COLUMN public.reservation_requests.rejected_at IS 'Set when request was rejected (for audit and CHECK).';

CREATE OR REPLACE FUNCTION public.set_rejected_at_on_reject()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  -- Only on transition to rejected; do not overwrite rejected_at if already set (backfill / manual fixes).
  IF NEW.status = 'rejected' AND (OLD.status IS NULL OR OLD.status IS DISTINCT FROM 'rejected') AND NEW.rejected_at IS NULL THEN
    NEW.rejected_at := now();
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER reservation_requests_set_rejected_at
  BEFORE UPDATE ON public.reservation_requests
  FOR EACH ROW
  EXECUTE FUNCTION public.set_rejected_at_on_reject();
...
-- =============================================================================
REVOKE ALL ON FUNCTION public.convert_request_to_reservation(uuid, uuid, integer, timestamptz, timestamptz, integer, text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.generate_or_regenerate_request_link_token(uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.generate_provider_request_token(uuid) FROM PUBLIC;
